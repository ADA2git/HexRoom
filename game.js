...
/* Hex Blast — core logic + rendering. Works in browser and Node (tests). */
...
const SHAPE_BY_ID = {};
for (let i = 0; i < SHAPES.length; i++) SHAPE_BY_ID[SHAPES[i].id] = SHAPES[i];
...
(function bootHexBlast() {
  ...
  const BEST_KEY = "hexblast-best";
  const HINT_KEY = "hexblast-hinted";
  ...
  function applyPlacement(piece, cells) {
    if (!H.placePiece(state.board, cells, piece.color)) return false;
    piece.placed = true;
  ...
  function drawWordmark(c) {
    ...
    if (y > layout.hudH + 8) {
      c.fillText("HEX BLAST", layout.boardX, y);
    }
    c.restore();
  }
